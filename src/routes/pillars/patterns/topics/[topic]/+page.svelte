<!-- src/routes/pillars/patterns/topics/[topic]/+page.svelte -->
<script lang="ts">
...
<SEO
  ...
  description={metadata?.description || 'Explore universal patterns.'}
  keywords={metadata?.keywords || 'universal perspective, patterns'}
  type="article"
...
<article class="min-h-screen bg-slate-900">
  ...
  <section class="border-t border-slate-800 bg-slate-950 px-6 py-12">
    <div class="mx-auto max-w-4xl">
      ...
      >
        <span>←</span>
        <span>{$currentLanguage === 'sv' ? 'Tillbaka till universella mönster' : 'Back to Universal Patterns'}</span>
      </a>
    </div>
  </section>

  <!-- Hero -->
  <section class="bg-gradient-to-b from-emerald-950 via-teal-950 to-slate-900 px-6 py-20">
    <div class="mx-auto max-w-4xl">
      {#if metadata?.icon}
      ...
      {#if metadata?.description}
        <p class="text-xl text-emerald-200">{metadata.description}</p>
      {/if}
    </div>
  </section>
  ...
  <section class="bg-slate-900 px-6">
    <div class="mx-auto max-w-4xl">
      <ShareButtons 
        title={metadata?.title || 'Universal Patterns'}
...
<style>
  ...
  :global(.prose) {
    ...
    --tw-prose-quote-borders: rgb(16 185 129);
