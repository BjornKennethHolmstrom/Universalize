<!-- src/routes/pillars/aesthetics/topics/[topic]/+page.svelte -->
<script lang="ts">
...
<article class="min-h-screen bg-slate-900">
  ...
  <section class="border-t border-slate-800 bg-slate-950 px-6 py-12">
    <div class="mx-auto max-w-4xl">
      <a 
        href="{base}/pillars/aesthetics" 
        class="inline-flex items-center gap-2 text-lg text-purple-300 transition hover:text-purple-200"
      >
        <span>←</span>
        <span>{$currentLanguage === 'sv' ? 'Tillbaka till universell estetik' : 'Back to Universal Aesthetics'}</span>
      </a>
    </div>
  </section>

  <!-- Hero -->
  <section class="bg-gradient-to-b from-purple-950 via-indigo-950 to-slate-900 px-6 py-20">
    <div class="mx-auto max-w-4xl">
      {#if metadata?.icon}
      ...
      {#if metadata?.description}
        <p class="text-xl text-purple-200">{metadata.description}</p>
      {/if}
    </div>
  </section>
